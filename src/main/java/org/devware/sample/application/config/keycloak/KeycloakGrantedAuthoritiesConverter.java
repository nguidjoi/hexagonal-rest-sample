...
public class KeycloakGrantedAuthoritiesConverter implements Converter<Jwt, Collection<GrantedAuthority>> {
	...
	public Collection<GrantedAuthority> convert(Jwt jwt) {
		var clientPermissions = clientPermissions(jwt);
		var clientRoles = clientRoles(jwt, clientId);

		Collection<GrantedAuthority> authorities = Stream.concat(clientPermissions.stream(), clientRoles.stream())
	...
	@SuppressWarnings("unchecked")
	protected List<String> clientPermissions(Jwt jwt) {

		return Optional.ofNullable(jwt.getClaimAsStringList(PERMISSIONS))
				.orElse(emptyList());
